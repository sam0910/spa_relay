...
class OTAUpdater:
    ...
    def get_latest_version(self):
        # https://api.github.com/repos/sam0910/spa_relay/releases/latest
        repos = "https://api.github.com/repos/{}/releases/latest".format(self.github_repo)
        print("Checking for latest version at", repos)
        latest_release = self.http_client.get(repos)
    ...
    def _download_all_files(self, version, sub_dir=""):
        ...
        for file in file_list_json:
            path = self.modulepath(
                ...
                + "/"
                + file["path"].replace(self.main_dir + "/", "").replace(self.github_src_dir, "")
            )
            if file["type"] == "file":
                gitPath = file["path"]
    ...
    def _download_file(self, version, gitPath, path):
        self.http_client.get(
            "https://raw.githubusercontent.com/{}/{}/{}".format(self.github_repo, version, gitPath), saveToFile=path
        )
